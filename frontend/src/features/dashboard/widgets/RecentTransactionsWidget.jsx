    ...
    return (
        <div className="glass-panel p-6 h-full flex flex-col">
            <div className="flex justify-between items-center mb-4 shrink-0">
                <h3 className="text-lg font-bold text-gray-800">{t('Recent Transactions')}</h3>
            ...
            <div className="space-y-4 overflow-y-auto flex-1 pr-1">
                {loading ? (
                    ...
                    <div className="text-center py-8 text-secondary text-sm">{t('No transactions found.')}</div>
                ) : (
                    transactions.map(tx => {
                            ...
                            <div key={tx.id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 -mx-2 px-2 rounded-lg transition-colors">
                                <div className="flex flex-col">
                                    <span className="font-medium text-gray-800 flex items-center gap-2">
                                        ...
                                        {tx.customer_id ? <span className="text-xs text-blue-500 font-normal">{t('Customer #')}{tx.customer_id}</span> : <span className="text-xs text-gray-400 font-normal">{t('Walk-in')}</span>}
                                    </span>
                                    <span className="text-xs text-gray-400">
                                        {/* Mock time or format created_at if available */}
                                        {tx.created_at ? new Date(tx.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : t('Just now')}
                                    </span>
                                </div>
